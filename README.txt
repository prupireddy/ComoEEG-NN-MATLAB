-Each of the programs have their own readme in the metadata section, are documented. This is more of a broad explanation. Process-oriented notes are stored in Report in the CNN + Spectrogram - Report
Brainstorming notes are stored in the Brainstorming folder in the same location's CNN V2 folder - but I am not sure if I have uploaded them yet.
...
considered ictal. You also have the option to use Tay's time diff calc style were you determine start stop then diff. The default is all both. 
...
Output: 3 options - 
A. P(insert patient number)_TFullPSD_176: Time difference
B. P(insert patient number)_TIFullPSD_176: Time Difference as well as with the 1 ictal = ictal window definition
C. P(insert patient number)_TNIFullPSD_176: Tay's Calculation-Style of Time Differential as well as with the 1 ictal = ictal window definition (default)

3. Boosting: Generates TIFF multitaper, same scale spectrograms (each page/layer is one channel's spectrogram in grayscale) for all ictals and an equal number of highest power interictals
Input: P(Insert patient number)_T(N)(I)FullPSD_176 - Features
